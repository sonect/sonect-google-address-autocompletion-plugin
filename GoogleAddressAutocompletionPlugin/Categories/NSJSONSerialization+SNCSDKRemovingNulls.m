//  NSJSONSerialization+SNCSDKRemovingNulls.m
//  Created by Richard Turton on 23/12/2013.

#import "NSJSONSerialization+SNCSDKRemovingNulls.h"

@implementation NSJSONSerialization (SNCSDKRemovingNulls)
...
+(id)sncsdk_JSONObjectWithData:(NSData *)data options:(NSJSONReadingOptions)opt error:(NSError *__autoreleasing *)error removingNulls:(BOOL)removingNulls ignoreArrays:(BOOL)ignoreArrays
{
    ...
    [JSONObject sncsdk_recursivelyRemoveNullsIgnoringArrays:ignoreArrays];
    return JSONObject;
}

@end

@implementation NSMutableDictionary (SNCSDKRemovingNulls)

-(void)sncsdk_recursivelyRemoveNulls
{
    [self sncsdk_recursivelyRemoveNullsIgnoringArrays:NO];
}
...
- (void)sncsdk_recursivelyRemoveNullsIgnoringArrays:(BOOL)ignoringArrays
{
    ...
    for (id arrayKey in arrayKeys)
    {
        ...
        [array sncsdk_recursivelyRemoveNullsIgnoringArrays:ignoringArrays];
    }
    ...
    for (id dictionaryKey in dictionaryKeys)
    {
        ...
        [dictionary sncsdk_recursivelyRemoveNullsIgnoringArrays:ignoringArrays];
    }
}

@end

@implementation NSMutableArray (SNCSDKRemovingNulls)
...
- (void)sncsdk_recursivelyRemoveNullsIgnoringArrays:(BOOL)ignoringArrays
{
    ...
    {
        [containedArray sncsdk_recursivelyRemoveNullsIgnoringArrays:ignoringArrays];
    }
    
    // Cascade down the dictionaries
    for (NSMutableDictionary * containedDictionary in [self objectsAtIndexes:dictionaryIndexes])
    {
        [containedDictionary sncsdk_recursivelyRemoveNullsIgnoringArrays:ignoringArrays];
    }
}
